...
% Load wind traces (eventually)

solarTraces=zeros(96,nDays);
...
while (col <= nDays/2)
    ...
    while (row <= 480)
        ...
        solarTraces(rowMod,col)=summer(row,colMod);
        row=row+1;
    end
    col=col+1;
end
...
if (col==61)
    while (col <= nDays)
        ...
        while (row <= 480)
            rowMod=mod(row-1,96)+1;
            solarTraces(rowMod,col)=winter(row,colMod);
            row=row+1;
        end
        col=col+1;
    end
